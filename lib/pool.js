...
var WILD_KECCAK_SCRATCHPAD_BUFFSIZE = 64 << 20;
...
var scratchpad = new DynamicBuffer(704);
var scratchpadSize = 0;
...
function processBlockTemplate(template){
	...
	if (scratchpadHeight.height == 0)
	{
		log('warn', logSystem, 'scratchpad empty, getting full scratchpad ', [scratchpadHeight.height]);
		getFullScratchpad(function (error, result) {
			if (error) {
				log('error', 'Job Refresher', 'Failed to get scratchpad: ' + error);
			} else {
				var scratchpadTmp = new Buffer(result.scratchpad_hex, 'hex');
				scratchpad.concat(scratchpadTmp);
				log('error', logSystem, 'scratchpad result: ', [scratchpadTmp.length]);
				scratchpadSize = scratchpadTmp.length;
				
				delete result.scratchpad_hex;
				scratchpadHeight = result.hi;
				scratchpad_helper = new ScratchpadHelper(scratchpadHeight);
				log('info', logSystem, 'scratchpad helper initialized at height: %d', [scratchpad_helper.hi.height]);
			}
		});
	
	return;
	}
	scratchpad_helper.fetchAddms(function (){
		if (currentBlockTemplate.height >  scratchpadHeight.height+1)
					{
						log('error', logSystem, 'height mismatch scratchpad: %d, currentBlockTemplate.height: %d, addms length: %d', [scratchpadHeight.height, currentBlockTemplate.height, scratchpad_helper.addms.length]);
						if(scratchpad_helper.addms.length != 0)
						{
							var addendums_added = 0;
							for(i =0;i<scratchpad_helper.addms.length;++i)
							{
								if(scratchpad_helper.addms[i].hi.height == scratchpadHeight.height + 1)
								{	
									addendums_added++;
									log('error', logSystem, scratchpad_helper.addms[i].addm);
									addmBuff = new Buffer(scratchpad_helper.addms[i].addm, 'hex');
									//log('error', logSystem, 'length of addendum: %d', [addmBuff.length]);
									scratchpad.concat(addmBuff);
									scratchpadSize += addmBuff.length;
									
									//log('error', logSystem, 'scratchpad ', [scratchpad]);
									//log('error', logSystem, 'addm ', [addmBuff]);
									//scratchpad = Buffer.concat([scratchpad, addmBuff]);
									scratchpadHeight.height = scratchpad_helper.addms[i].hi.height;
									scratchpadHeight.block_id = scratchpad_helper.addms[i].hi.block_id;
									log('error', logSystem, 'new scratchpad height: %d, currentBlockTemplate.height: %d', [scratchpadHeight.height, currentBlockTemplate.height]);
									scratchpad_helper.popAddms();
								}
						...
						{
							log('error', logSystem, 'failed to get addendums, downloading full scratchpad');
							getFullScratchpad(function (error, result) {
								if (error) {
									log('error', 'Job Refresher', 'Failed to get scratchpad: ' + error);
								} else {
...
//					if (currentBlockTemplate.height > scratchpadHeight.height+1)
//					{
//						//log('error', logSystem, scratchpad.toString('hex'));
//						log('error', logSystem, 'height mismatch scratchpad: %d, currentBlockTemplate.height: %d', [scratchpadHeight.height, currentBlockTemplate.height]);
//						for(i =0;i<miner.addms.length;++i)
//							{
//								if(miner.addms[i].hi.height == scratchpadHeight.height + 1)
//								{
//									copyBuff = new Buffer(miner.addms[i].addm, 'hex');
//									//log('error', logSystem, 'addm ', [copyBuff]);
//									scratchpad = Buffer.concat([scratchpad, copyBuff]);
//									scratchpadHeight.height = miner.addms[i].hi.height;
//									scratchpadHeight.block_id = miner.addms[i].hi.block_id;
//									log('error', logSystem, 'new scratchpad height: %d, currentBlockTemplate.height: %d', [scratchpadHeight.height, currentBlockTemplate.height]);
//								}
//
//							}
...
function ScratchpadHelper(Height)
{
	this.hi = {block_id: Height.block_id, height: Height.height};
    this.addms = [];
}

ScratchpadHelper.prototype = {
	getJob: function(){
    ...
    fetchAddms: function(callback) {
        if(this.hi.height === 0
			...
			log('error', logSystem, 'not downloading addendums addendums, height: %d', [this.hi.height]);
            return callback();
        }
        var miner = this;
        getAddms(this.hi, function(error, addms) {
            ...
            miner.addms = [];
			log('error', logSystem, 'scratchpad addendums downloaded: %d', [addms.length]);
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
		...
		hash = multiHashing.boolberry(convertedBlob, scratchpad.getBuffer(), job.height, scratchpadSize);
        shareType = 'valid';
    }
    //BBR FIXME:
